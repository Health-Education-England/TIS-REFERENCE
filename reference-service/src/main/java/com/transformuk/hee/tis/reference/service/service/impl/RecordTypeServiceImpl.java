...
public class RecordTypeServiceImpl {
  ...
  public Page<RecordType> advancedSearch(String searchString, List<ColumnFilter> columnFilters, Pageable pageable) {

    List<Specification<RecordType>> specs = new ArrayList<>();
    ...
    Page<RecordType> result;
    if (!specs.isEmpty()) {
      Specifications<RecordType> fullSpec = Specifications.where(specs.get(0));
      //add the rest of the specs that made it in
      for (int i = 1; i < specs.size(); i++) {
        fullSpec = fullSpec.and(specs.get(i));
      }
      result = recordTypeRepository.findAll(fullSpec, pageable);
    } else {
      result = recordTypeRepository.findAll(pageable);
    }
