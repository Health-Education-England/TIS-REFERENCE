...
public class CountryServiceImpl {
  ...
  public Page<Country> advancedSearch(String searchString, List<ColumnFilter> columnFilters, Pageable pageable) {
    ...
    Page<Country> result;
    if (!specs.isEmpty()) {
      Specifications<Country> fullSpec = Specifications.where(specs.get(0));
      //add the rest of the specs that made it in
      for (int i = 1; i < specs.size(); i++) {
        fullSpec = fullSpec.and(specs.get(i));
      }
      result = countryRepository.findAll(fullSpec, pageable);
    } else {
