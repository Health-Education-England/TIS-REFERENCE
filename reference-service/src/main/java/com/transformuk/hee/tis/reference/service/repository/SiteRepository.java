...
public interface SiteRepository extends JpaRepository<Site, Long>, JpaSpecificationExecutor {
  ...
  @Query("SELECT s " +
      ...
      "or s.siteName like %:param% or s.address like %:param% or s.postCode like %:param% ) " +
      "AND s.status = :status")
  Page<Site> findByStatusAndSearchString(@Param("status") Status status,
  ...
  @Query("SELECT s FROM Site s WHERE s.trustCode =:trust and (s.siteCode like %:param% " +
      "or s.siteName like %:param% or s.address like %:param% or s.postCode like %:param%)")
  List<Site> findBySearchStringAndTrustCode(@Param("trust") String trustCode,
      @Param("param") String searchString,
      Pageable pageable);
  ...
  List<String> findSiteTrustMatch(@Param("siteCode") String siteCode,
