...
public class TrainingNumberTypeServiceImpl {
  ...
  public Page<TrainingNumberType> advancedSearch(String searchString, List<ColumnFilter> columnFilters, Pageable pageable) {
    ...
    Page<TrainingNumberType> result;
    if (!specs.isEmpty()) {
      Specifications<TrainingNumberType> fullSpec = Specifications.where(specs.get(0));
      //add the rest of the specs that made it in
      for (int i = 1; i < specs.size(); i++) {
        fullSpec = fullSpec.and(specs.get(i));
      }
      result = trainingNumberTypeRepository.findAll(fullSpec, pageable);
    } else {
      result = trainingNumberTypeRepository.findAll(pageable);
    }
